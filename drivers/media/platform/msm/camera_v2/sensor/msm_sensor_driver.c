	...
	struct msm_sensor_info_t *probed_info, char *entity_name)
{
	...
	for(i=0; i<CAMERA_VENDOR_EEPROM_COUNT_MAX; i++){
		...
		if(strcmp(slave_info->eeprom_name,s_vendor_eeprom[i].eeprom_name) == 0){
			if(((strcmp(slave_info->sensor_name,"s5k2l8_ofilm_zangyapro") == 0) && (s_vendor_eeprom[i].module_id == MID_OFILM))
			|| ((strcmp(slave_info->sensor_name,"s5k3m3sm") == 0) && (s_vendor_eeprom[i].module_id == MID_OFILM))
			|| ((strcmp(slave_info->sensor_name,"imx350_ofilm_zangyapro") == 0) && (s_vendor_eeprom[i].module_id == MID_OFILM))
			|| ((strcmp(slave_info->sensor_name,"s5k4h7yx_ofilm_zangya") == 0) && (s_vendor_eeprom[i].module_id == MID_OFILM))
